 ...
 * @param button {GamepadButton || number}
 * @return {boolean}
 */
 ...
 * @param gp {Gamepad}
 * @param buttonIndexes {number[]} - Array of indexes for {@link Gamepad.buttons}
 * @param isAny {boolean} - If `true` (by default) – {@link buttonsPressed} return `true`
 ...
 * @param axes {number[]}
 * @param moreOrLess {'more' | 'less'}
 * @param value {number}
 * @return {boolean}
 */
const axesToBoolean = (axes, moreOrLess, value) => {
    const ratio = moreOrLess === 'more' ? -1 : 1;
    return axes.reduce((acc, val) => acc || ratio * val > -ratio * value, false);
};
...
const ShowerActionOnButton = {
    ...
    /** @param shower {Shower} */
    toggleFull: (shower) => (shower.isFullMode ? shower.exitFullMode() : shower.enterFullMode()),
    /** @param shower {Shower} */

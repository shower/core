...
class Next {
    ...
    _getElements() {
        ...
        return Array.from(slideEl.querySelectorAll(this._elementsSel));
    }

    _getInnerComplete() {
        return this._elements
            .filter(el => el.classList.contains(this._activeClass))
            .length;
    }
    ...
    _toggleActive() {
        this._elements.forEach((el, index) => {
            el.classList.toggle(this._activeClass, index < this._innerComplete);
        });
    }

...
modules.define('Plugins', [
    ...
    extend(Plugins.prototype, /** @lends shower.Plugins.prototype */ {
        ...
        get: function (name, shower) {
            ...
            if (plugin && shower) {
                for (var i = 0, l = this._instances.length; i < l; i++) {
                    ...
                    if (instanceInfo.plugin.name === name && instanceInfo.shower === shower) {
                        return instanceInfo.instance;
                    }
                }
            }

            return plugin && plugin.class;
        },

        _requireAndAdd: function (plugin) {
            modules.require(plugin.name, function (pluginClass) {
        ...
        _instance: function (plugin, shower) {
            var options = plugin.options || shower.options.get('plugin:' + plugin.name);
